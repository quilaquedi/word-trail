...
function loadTextFromDb(textId: number) {
	...
	}).from(word).where(eq(word.textId, textId)).orderBy(word.textPos).all();
	return result
}
...
function loadContexts(wordId: string | null) {
	...
	switch (wordId) {
		...
		case 'B':
			...
			break;
		case 'D3':
			context = { compId: 'B', text: 'This is a text. That sea is blue. Click on the second word.', wordLoc: [5, 7] };
			contexts = { same: [context], spelling: [], meaning: [] };
			break;
		default:
			contexts = { same: [], spelling: [], meaning: [] };
